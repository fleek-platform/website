...
import { DynamicProvider } from './auth/DynamicProvider.tsx';
    ...
    triggerUserLogin: async (): Promise<boolean> => {
      try {
        ...
        return await new Promise<boolean>((resolve) => {
          if (loginPromiseResolveRef) {
            console.log('✅ Setting loginPromiseResolveRef.current');
            loginPromiseResolveRef.current = resolve;
          } else {
            console.log('❌ loginPromiseResolveRef is undefined');
            resolve(false);
          }
        });
      } catch (error) {
...
const Eliza: React.FC = () => {
  const loginPromiseResolveRef = useRef<(value: boolean) => void>();

  return (
    <AuthPromiseContext.Provider value={{ loginPromiseResolveRef }}>
      <DynamicProvider>
        <CoreEliza />
      </DynamicProvider>
    </AuthPromiseContext.Provider>
  );
};
