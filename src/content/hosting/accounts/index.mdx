...
Fleek uses usernames to identify users we will be using this in internal communications and also in the UI. The username can be changed at any time from the 'Account Settings' page.
...
:::warn
Ensure that you copy and store your API key in a safe and secure manner. This key will be revealed only once, and if lost, might not be retrievable. Taking precautionary measures now can prevent potential complications or security breaches in the future.
:::

To get an API key:

1. Navigate to your [Fleek project's](https://fleek.xyz/dashboard/) dashboard.
...
5. On this page you will find a list of API keys.
...
Remove an API key by clicking the three-dot menu (⋮) in its table row and selecting "Delete" from the popup options. Remember that deleting the API key will immediately stop any services that rely on it.
