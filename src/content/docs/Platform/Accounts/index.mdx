...
API keys are a type of token that provides access to specific Fleek API endpoints. You can create as many keys as you need and can delete them at any time.

### Creating an API key

To create an API key start by visiting 'Account settings' and clicking on the 'API Keys' section. Click 'Create' and choose a name that will identify this API key. You will be prompted to confirm the creation by completing the authentication process. You will then receive an API key that you can use to authenticate with Fleek's services.

:::warn
Ensure that you diligently copy and store your API key in a safe and secure manner. This key will be revealed only once, and if lost, might not be retrievable. Taking precautionary measures now can prevent potential complications or security breaches in the future.
:::

### Deleting an API Key

To delete an API key, start by visiting 'Account settings' and clicking on the 'API Keys' section. Click on the 'Delete' button next to the key you want to delete. Once deleted, all resources using the API key will stop working.
