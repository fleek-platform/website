...
You can check out the [templates](/templates) to see frameworks we have support for. Fleek supports a wide range of frameworks, spanning from the most popular to the less popular ones.
...
1. Get the [`fleek-next-cli`](/docs/cli/fleek-next-cli)
...
- **Service Integrations**: Fleek provides integrations with popular services like MongoDB, Sanity, and more, streamlining environment setup and configuration. We offer [Edge SGX](/docs/cli/edge-sgx) for data security and privacy that can be paired with these external services.
