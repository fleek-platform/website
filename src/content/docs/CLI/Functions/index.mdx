...
Assets are files that are uploaded alongside your Fleek Function to IPFS and can be accessed by your function code. You can access the IPFS CID of the assets using the `fleek.env.ASSETS_CID` environment variable.

These assets are useful for storing static files that your function code needs to access, such as images, videos, or other resources. You can upload assets by passing the path to the assets directory using the `--assets <path>` flag when deploying your Fleek Function.

```sh
fleek functions deploy --name my-function --path ./function.js --assets ./assets
```

To fetch the assets in your function code, you can make a simple HTTP request to our IPFS gateway:

```ts
// Example: Fetching an image from your assets
const response = await fetch(
  `https://${fleek.env.ASSETS_CID}.flk-ipfs.xyz/images/logo.png`,
);
const imageData = await response.arrayBuffer();
```

:::note
The assets are stored on IPFS, so they are publicly accessible by default. Support for private assets is coming soon.
:::
...
Your Fleek Functions are stored on IPFS by default, making them [publicly accessible](https://docs.ipfs.tech/concepts/privacy-and-encryption/). This will expose all your source code, including any environment variables bundled with it.
Ensure you are not inadvertently exposing data you are interested in keeping private.
If you need to upload sensitive data with your Fleek Function, you can make use of the `--private` flag when deploying and your Fleek Function will be stored on Fleek's private storage, which is not exposed to the public.
