...
Assets are files that are uploaded alongside your Fleek Function to IPFS and can be accessed by your function code. You can access the IPFS CID of the assets using the `process.env.ASSETS_CID` environment variable.
...
To fetch the assets, you can make a simple HTTP request to our ipfs gateway with `fetch`:

```sh
fetch(`https://${fleek.env.ASSETS_CID}.flk-ipfs.xyz/path/to/asset`);
```
